...
go_data <- read_xlsx("zGreenExperiment.xlsx", sheet = "DAVIDGeneOntology")
...
top_go_data <- top_go_data %>%
  mutate(ordered_go_term = factor(`Biological Process GO Term`, levels = `Biological Process GO Term`[order(-pvalue)]))

# Plot
dot_plot <- ggplot(top_go_data, aes(x = -log10(pvalue), y = ordered_go_term, size = Count, color = Enrichment)) +
  geom_point() +
...
output_file <- "zGreenDAVID.png"  # Update with the desired output file path
